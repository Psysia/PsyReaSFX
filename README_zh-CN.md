...
<p align="center">
  <img src="https://img.shields.io/badge/版本-0.7.22--beta.26-19D8FF" alt="0.7.22 beta 26">
...
主工作区新增常驻的 **Path 路径栏**。展开后可以在同一棵树中浏览全部
逻辑库、实体来源和已经索引的真实子目录。箭头只负责展开或折叠，单击
名称会立即显示该目录及全部下级目录中的音频。当前路径会保留为可清除
的筛选条件，目录展开状态也会在下次启动时恢复。

层级树直接读取现有素材索引，并按小批次在后台建立。展开目录不会重新
扫描硬盘；不包含已索引音频的空目录不会显示。
...
`0.7.22 Beta 26` 新增索引驱动的文件夹层级浏览，并继续进行 0.7 浏览、
