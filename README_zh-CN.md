...
随后执行 `扩展 → ReaPack → 同步软件包`，直接搜索需要的工具并安装即可。使用 Beta 7.10 时，先为软件包启用
...
| 分类 | 工具 | 用途 |
|---|---|---|
| 渲染 | 创建文件夹区域与渲染矩阵 | 单一源轨道直接使用自身命名并勾选自身；多轨时才使用最近公共父级。 |
...
| 显示 | 视频素材轨道名覆盖显示 | 在视频 Item 内显示当前轨道名称，需要时可直接切换到文件名模式。 |
| 显示 | 视频素材源文件名覆盖显示 | 在视频 Item 内显示源视频文件名并隐藏扩展名，可随时直接切回轨道名模式。 |
...
|---|---|---|
| 稳定版 | **0.8.5** | 推荐日常使用。 |
| 预发布版 | **0.9.0 Beta 7.10** | 当前预发布线；支持 Windows 超长路径 PCM/float WAV 波形。 |
...
- **预发布通道：0.9.0 Beta 7.10** — 为 Windows 超长路径 PCM/float WAV 增加分帧直接波形读取，不再依赖失败的 REAPER 峰值缓存。
...
Beta 7.10 包含独立的 **AI 语义搜索**。它与“查找相似声音”完全分离：顶部搜索框按 `Enter` 执行普通搜索，点击金色 AI 按钮执行语义搜索。AI 只把自然语言描述转换为中英文检索词；素材召回、排序和每批 120 条的分页追加均在本地完成。API 不再接收候选元数据，因此继续加载下一批无需等待网络，也不会再次产生 API 请求。音频、素材文件名、元数据、目录和文件路径均不会上传。
...
- 独立 AI 语义搜索入口：支持 DeepSeek、OpenAI 与 OpenAI-compatible API，由 AI 扩展查询词并在本地完成素材召回与排序。
...
- [PsyReaSFX 0.9.0 Beta 7.10 预发布](https://github.com/Psysia/PsyReaSFX/releases/tag/v0.9.0-beta7.10)
- [Beta 7.10 可选神经组件手动 ZIP 备用下载](https://github.com/Psysia/PsyReaSFX/releases/download/v0.9.0-beta7.10/PsyReaSFX_Neural_Similarity_v0_9_0_beta7_10_win_x64.zip)
