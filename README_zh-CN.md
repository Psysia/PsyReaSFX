...
随后执行 `扩展 → ReaPack → 同步软件包`，直接搜索需要的工具并安装即可。使用 Beta 7.5 时，先为软件包启用
...
|---|---|---|
| 稳定版 | **0.8.5** | 推荐日常使用。 |
| 预发布版 | **0.9.0 Beta 7.5** | 当前预发布线；增强 DeepSeek 结构化输出与自动恢复能力。 |
...
- **预发布通道：0.9.0 Beta 7.5** — 兼容 DeepSeek 默认思考模式、被文字包裹的 JSON 和偶发空结构化响应。
...
Beta 7.5 包含独立的 **AI 语义搜索**。它与“查找相似声音”完全分离：顶部搜索框按 `Enter` 执行普通搜索，点击金色 AI 按钮则直接按当前文字执行语义搜索。DeepSeek 结构化请求会明确关闭默认思考模式，可解析被说明文字、代码块或内容分块包裹的有效 JSON，并在 JSON Output 返回空内容或格式异常时自动重试一次。AI 搜索仍不受波形与目录维护任务阻断。音频和完整文件路径不会上传；API 只会收到查询，以及本地召回出的最多 120 条候选文本元数据。
...
- [PsyReaSFX 0.9.0 Beta 7.5 预发布](https://github.com/Psysia/PsyReaSFX/releases/tag/v0.9.0-beta7.5)
- [Beta 7.5 可选神经组件手动 ZIP 备用下载](https://github.com/Psysia/PsyReaSFX/releases/download/v0.9.0-beta7.5/PsyReaSFX_Neural_Similarity_v0_9_0_beta7_5_win_x64.zip)
