...
> **开发预览：**PsyReaSFX 0.8.0 Beta 3 将原计划 Beta 3/4/5 合并完成：
> 缺失文件检查与来源重定位、面向大型素材库的采样式重复检测，以及
> REAPER 工程使用记录与项目素材箱绑定。它同时保留 Beta 1/2 的中断恢复、
> 数据备份、缓存校验和静默 Watch Folder。ReaPack 默认仍安装 Stable，
> 测试版需要用户主动启用预发布通道。
...
将鼠标悬停在搜索框旁的无边框“文件夹搜索”图标上，即可通过原生级联
菜单按“逻辑库 → 实体来源 → 已索引子目录”逐级横向浏览。单击任意层级
会立即显示该目录及全部下级目录中的音频，菜单随后收起，只在结果区
上方保留一条紧凑、可清除的 `Pathname` 条件。

层级直接读取现有素材索引，并按小批次在后台建立。悬停浏览不会重新
扫描硬盘，也不会常驻占用工作区高度；不包含已索引音频的空目录不会
显示。
...
PsyReaSFX 不会一次把整个素材库塞进界面。扫描、文件夹层级建立、元数据、
波形建立和 Artwork 发现都被拆分为小任务；可见行和当前选中素材优先
处理。波形写入磁盘缓存，也可以在设置中预缓存整个库，以便大型素材库
获得可预期的滚动与预览速度。
