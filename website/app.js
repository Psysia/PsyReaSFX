...
const messages = {
  en: {
    ...
    neuralInstallNote: "Enable pre-release versions, install PsyReaSFX.lua Beta 6.1, then enable/install the separate PsyReaSFX Neural Similarity package on Windows x64. ReaPack updates both.",
    ...
    aiSemanticSearch: "In development: describe a sound in natural language and rank local metadata with DeepSeek or an OpenAI-compatible API",
    ...
    installLead: "Import and synchronize the repository, enable package pre-releases, and install PsyReaSFX.lua Beta 6.1. On Windows x64, repeat for the optional PsyReaSFX Neural Similarity package. Both update through the same URL.",
    stepOne: "Open Extensions → ReaPack → Import repositories…",
    stepTwo: "Paste the repository URL below.",
    stepThree: "Enable pre-releases and install PsyReaSFX.lua Beta 6.1; on Windows x64, repeat for PsyReaSFX Neural Similarity.",
    copy: "Copy",
    copied: "Copied",
    host: "Host",
    required: "Required",
  ...
  zh: {
    ...
    neuralInstallNote: "先启用预发布并安装 PsyReaSFX.lua Beta 6.1；Windows x64 如需神经检索，再为独立包 PsyReaSFX Neural Similarity 启用预发布并安装。两个包都由 ReaPack 更新。",
    ...
    aiSemanticSearch: "开发中：用自然语言描述声音，通过 DeepSeek 或 OpenAI-compatible API 对本地元数据候选排序",
    ...
    installLead: "在 ReaPack 中导入并同步仓库，为软件包启用预发布后安装 PsyReaSFX.lua Beta 6.1；Windows x64 用户再对可选包 PsyReaSFX Neural Similarity 执行同样操作。两个包都通过同一地址更新。",
    stepOne: "打开 Extensions → ReaPack → Import repositories…",
    stepTwo: "粘贴下方仓库地址。",
    stepThree: "启用预发布并安装 PsyReaSFX.lua Beta 6.1；Windows x64 如需神经检索，再对 PsyReaSFX Neural Similarity 执行同样操作。",
    copy: "复制",
    copied: "已复制",
    host: "宿主",
    required: "必需",
