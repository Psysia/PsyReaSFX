...
const PREVIEW_FALLBACK = {
  version: "0.9.0 Beta 7.2",
  url: "https://github.com/Psysia/PsyReaSFX/releases/tag/v0.9.0-beta7.2"
};
...
const messages = {
  en: {
    ...
    neuralInstallNote: "Enable pre-release versions and install PsyReaSFX.lua Beta 7.2. On Windows x64, install the matching optional PsyReaSFX Neural Similarity package if needed.",
    ...
    findText: "Beta 7.2 updates DeepSeek to the current deepseek-flash and deepseek-v4-pro API models, fixes clipped AI settings fields, and shows API-key save results inline.",
    ucsNavigation: "Browse Category, SubCategory and CatID without moving source files",
    aiSemanticSearch: "Describe a sound in natural language and rank local metadata with DeepSeek or an OpenAI-compatible API",
    ...
    installLead: "Import and synchronize the repository, enable package pre-releases, and install PsyReaSFX.lua Beta 7.2. On Windows x64, repeat for the matching optional PsyReaSFX Neural Similarity package. Both update through the same URL.",
    stepOne: "Open Extensions → ReaPack → Import repositories…",
    stepTwo: "Paste the repository URL below.",
    stepThree: "Enable pre-releases and install PsyReaSFX.lua Beta 7.2; on Windows x64, repeat for PsyReaSFX Neural Similarity if needed.",
    copy: "Copy",
    copied: "Copied",
    host: "Host",
    required: "Required",
  ...
  zh: {
    ...
    neuralInstallNote: "先启用预发布并安装 PsyReaSFX.lua Beta 7.2；Windows x64 如需神经检索，再安装配套的可选包 PsyReaSFX Neural Similarity。",
    ...
    findText: "Beta 7.2 将 DeepSeek 更新到当前 deepseek-flash / deepseek-v4-pro API 模型，修复 AI 设置裁切，并在页面内显示 API Key 保存结果。",
    ucsNavigation: "按 Category、SubCategory 和 CatID 浏览，不移动源文件",
    aiSemanticSearch: "用自然语言描述声音，通过 DeepSeek 或 OpenAI-compatible API 对本地元数据候选排序",
    ...
    installLead: "在 ReaPack 中导入并同步仓库，为软件包启用预发布后安装 PsyReaSFX.lua Beta 7.2；Windows x64 用户如需神经检索，再安装配套的可选包 PsyReaSFX Neural Similarity。两个包都通过同一地址更新。",
    stepOne: "打开 Extensions → ReaPack → Import repositories…",
    stepTwo: "粘贴下方仓库地址。",
    stepThree: "启用预发布并安装 PsyReaSFX.lua Beta 7.2；Windows x64 如需神经检索，再对 PsyReaSFX Neural Similarity 执行同样操作。",
    copy: "复制",
    copied: "已复制",
    host: "宿主",
    required: "必需",
