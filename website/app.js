...
const PREVIEW_FALLBACK = {
  version: "0.9.0 Beta 7.5",
  url: "https://github.com/Psysia/PsyReaSFX/releases/tag/v0.9.0-beta7.5"
};
...
const messages = {
  en: {
    ...
    neuralInstallNote: "Enable pre-release versions and install PsyReaSFX.lua Beta 7.5. On Windows x64, install the matching optional PsyReaSFX Neural Similarity package if needed.",
    ...
    findText: "Beta 7.5 improves DeepSeek JSON Output reliability by disabling default thinking for structured requests, accepting wrapped JSON, and retrying an occasional empty response once.",
    ...
    installLead: "Import and synchronize the repository, enable package pre-releases, and install PsyReaSFX.lua Beta 7.5. On Windows x64, repeat for the matching optional PsyReaSFX Neural Similarity package. Both update through the same URL.",
    stepOne: "Open Extensions → ReaPack → Import repositories…",
    stepTwo: "Paste the repository URL below.",
    stepThree: "Enable pre-releases and install PsyReaSFX.lua Beta 7.5; on Windows x64, repeat for PsyReaSFX Neural Similarity if needed.",
    copy: "Copy",
    copied: "Copied",
    host: "Host",
    required: "Required",
  ...
  zh: {
    ...
    neuralInstallNote: "先启用预发布并安装 PsyReaSFX.lua Beta 7.5；Windows x64 如需神经检索，再安装配套的可选包 PsyReaSFX Neural Similarity。",
    ...
    findText: "Beta 7.5 通过关闭结构化请求的默认思考模式、兼容外层包裹 JSON，并对偶发空响应自动重试一次，提升 DeepSeek 搜索稳定性。",
    ...
    installLead: "在 ReaPack 中导入并同步仓库，为软件包启用预发布后安装 PsyReaSFX.lua Beta 7.5；Windows x64 用户如需神经检索，再安装配套的可选包 PsyReaSFX Neural Similarity。两个包都通过同一地址更新。",
    stepOne: "打开 Extensions → ReaPack → Import repositories…",
    stepTwo: "粘贴下方仓库地址。",
    stepThree: "启用预发布并安装 PsyReaSFX.lua Beta 7.5；Windows x64 如需神经检索，再对 PsyReaSFX Neural Similarity 执行同样操作。",
    copy: "复制",
    copied: "已复制",
    host: "宿主",
    required: "必需",
