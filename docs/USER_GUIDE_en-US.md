...
**Applies to:** PsyReaSFX 0.8.5 Stable / 0.9.0 Beta 7.5
**Author:** Psysia  
**Host:** REAPER 7.x
...
4. Search for `PsyReaSFX.lua`, right-click it, enable pre-release versions for
   that package, choose `0.9.0-beta7.5` under **Versions**, and install it.
...
### Optional neural similarity component (Beta 7.5)
...
choose `0.9.0-beta7.5`, and install it beside `PsyReaSFX.lua`. ReaPack places
...
In Beta 7.5, AI search uses an independent background-job resource and captures its asset-array snapshot at launch. Long-running waveform precaching, cache verification, catalog snapshots, and other catalog maintenance therefore no longer cause `resource_busy`. DeepSeek requests also disable its default thinking mode; if JSON Output occasionally returns empty or malformed content, PsyReaSFX retries once with a stricter prompt.
...
With the optional Beta 7.5 component installed, PsyReaSFX uses EfficientAT
...
PsyReaSFX 0.9.0 Beta 7.5 as versions of the same package. Normal synchronization
