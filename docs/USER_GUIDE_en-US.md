...
**Applies to:** PsyReaSFX 0.8.5 Stable / 0.9.0 Beta 7.10
**Author:** Psysia  
**Host:** REAPER 7.x
...
4. Search for `PsyReaSFX.lua`, right-click it, enable pre-release versions for
   that package, choose `0.9.0-beta7.10` under **Versions**, and install it.
...
### Optional neural similarity component (Beta 7.10)
...
choose `0.9.0-beta7.10`, and install it beside `PsyReaSFX.lua`. ReaPack places
...
Usage guidance, service state, privacy scope, and API configuration are collected under `Settings → AI Search`. Before the first search, configure a provider there:

- `DeepSeek`: defaults to `https://api.deepseek.com/chat/completions` and `deepseek-flash`; the more capable `deepseek-v4-pro` is also available. Saved `deepseek-chat` / `deepseek-reasoner` settings migrate automatically.
...
Each search has two stages. The AI converts the request into bilingual sound concepts once. PsyReaSFX then scans, ranks, and keeps up to 2,400 candidates locally. The first 120 are displayed; **Load next 120** appends another page immediately from memory. Only the search description is sent to the API. Filenames, Description, Keywords, UCS fields, directories, file paths, and audio stay local. Results are ordered by local recall score, then path for stable ties.
...
With the optional Beta 7.10 component installed, PsyReaSFX uses EfficientAT
...
- Beta 7.10 reads PCM/float WAV waveforms directly when a Windows path is 260
  characters or longer. After updating, use `Settings → Reliability & recovery
  → Retry all failed tasks` to process old failures again. This does not copy,
  transcode or modify source files.
...
PsyReaSFX 0.9.0 Beta 7.10 as versions of the same package. Normal synchronization
