...
- `AppState` 是新模块的状态修改入口，可在纯 Lua 测试中注入隔离状态表。
- `Host` 是 REAPER、SWS 与 ReaImGui 的可注入适配器；存储、目录、分析和重复
  确认模块通过 `HostApi` 调用宿主能力，纯模块测试不再需要启动 REAPER。
- `tools/audit_lua_architecture.py` 在 CI 中禁止核心模块重新直接调用 `reaper`，并
  锁定旧模块直接写 `state` 的数量上限；新增代码必须使用边界，旧债只能减少。
- 英文界面文案继续集中在 `I18N_EN`、前缀和模式资源中；英文模式遇到未覆盖的
  中文文本会登记到运行时诊断，便于发布前审计。

## Lua 发布源

Lua 的可维护源已按职责机械拆分到 `src/lua/`，顺序为启动与常量、UI 核心、状态
入口、宿主适配、任务与存储、目录、增量算法、分析和运行时 UI。
`tools/Build-LuaRelease.ps1` 按固定清单合并模块，并统一写入 `@version`，最终仍
产出 ReaPack 所需的单个 Lua 文件；拆分脚本已同步支持全部 13 个模块的往返校验。
