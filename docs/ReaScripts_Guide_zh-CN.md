...
v1.4 不再把 Folder 父轨当作普通单轨移动，而是把 REAPER 的轨道结构按“树”处理。

脚本会自动找到当前目标的**最近共同父级**，然后在这个层级上按完整单元排序：

- 普通轨道作为单轨单元移动；
- Folder 父轨会连同它下面的全部子轨、嵌套 Folder 一起移动；
- 原有嵌套层级不会被拆散；
- 未参与排序的同级轨道或 Folder 保持原来的槽位。

### 选中素材模式

只要存在选中素材，就优先使用素材模式。

脚本根据这些素材所在轨道自动判断应在哪一层排序，并以每个排序单元内部**最早的选中素材**作为排序依据。

例如：

- 素材位于两个普通同级轨道 → 排这两条轨道；
- 素材分别位于两个同级 Folder 内 → 两个完整 Folder 作为整体排序；
- 素材位于不同嵌套分支 → 自动找到最近共同父级，在该层移动对应的完整子树。

### 选中轨道模式

没有选中素材时，使用当前选中的轨道作为目标。

如果目标对应 Folder，则按完整 Folder 子树处理；排序位置取该单元整个子树中最早的素材。

### 安全规则

Folder 父轨不会再被单独抽出来移动。脚本每次移动 Folder 时都会把整棵子树一起选中并作为一个连续块移动，从而避免排序后文件夹被解除或内部层级错乱。
...
在选中内容上方创建新的 Folder Bus，并自动挂载 Pro-L 2。

v1.1 同时支持普通轨道和已经存在的文件夹：

1. 选择连续的普通同级轨道，可以像原来一样直接包成一个新 Folder；
2. 只选中已有文件夹的父轨时，会自动把该文件夹的完整子树包含进去；
3. 同时选择多个相邻、同级的已有文件夹，可以在它们外面再建立一个新的父级；
4. 原文件夹内部的层级和结束关系保持不变；
5. 完成后自动选中新建的 Folder Bus。

### 选择规则

作为新 Folder 直接子级的选中对象必须位于同一个直接父级下，并且在把已有文件夹展开为完整子树后组成连续的一块。

选择已有文件夹时，**不需要再把里面所有子轨一起选中**，只选文件夹父轨即可。

如果选中对象跨越不同父级，或中间夹着不属于选择范围的无关轨道，脚本会停止，不会把这些轨道误包进新父级。

### 前提

REAPER FX Browser 必须能够通过以下名称找到插件：
...
已经做好几个独立音效 Folder 后，同时选中这些 Folder 的父轨，一键在外面再建立一个更高一级的 Bus，同时保留所有原有内部结构。
...
- **外层选中文件夹保持完全展开**；
- 只有内部嵌套的子文件夹使用 REAPER 的“小折叠”状态；
- 直接属于外层文件夹的普通轨道保持正常高度，不会跟着一起小折叠。
...
**Rename Selected Tracks from Parent Folder Name**  
当前版本：**1.0**
...
同一父级下按轨道从上到下编号。如果同时选中了多个不同父级下的轨道，每个父级都会独立从 `_01` 开始。

### 嵌套文件夹

嵌套结构同样只认最近一级父文件夹，不会继续向更高层祖先查找。
...
没有父级文件夹的轨道，或者直接父级没有名称的轨道，会自动跳过。

### 推荐使用方式

整理好 Folder 层级后，选中需要统一命名的子轨道执行一次，即可快速继承父级命名规则。
