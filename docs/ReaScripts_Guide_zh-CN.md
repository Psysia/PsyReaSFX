...
脚本会根据当前选中的最外层文件夹内部是否存在下级文件夹，自动选择“两状态”或“四状态”。

#### 无嵌套文件夹：两状态

如果所有选中的最外层文件夹下面都只有普通轨道：
...
- **深度完全展开**：最外层文件夹和内部所有嵌套文件夹全部展开，所有轨道都可见。
- **收起子文件夹内轨道**：最外层文件夹保持展开，但所有下级文件夹都完全折叠，只显示子文件夹本身，不显示它们内部的轨道。
- **小折叠**：整个文件夹层级统一进入 REAPER 的小折叠状态。
- **完全折叠**：最外层文件夹和内部文件夹全部完全折叠。

这样四个状态都会产生明确不同的视觉结果，不再出现之前“普通展开”和“深度完全展开”重合、看起来只有三层的问题。

### 多文件夹同时选中

只要任意一个最外层选中文件夹存在嵌套，整组选中对象统一走四状态；只有全部都是单层文件夹时，整组才使用两状态。

### 推荐使用方式

仍然只需要绑定一个快捷键：单层 Folder 是快速“展开 / 折叠”开关，嵌套 Folder 自动使用四层结构切换。
