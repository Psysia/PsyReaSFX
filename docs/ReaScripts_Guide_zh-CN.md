...
把所有选中的文件夹父轨统一切换到同一个折叠级别，并按以下顺序循环：

```text
完全展开 → 小折叠 → 完全折叠 → 完全展开
```

普通轨道即使被选中也不会修改，只处理真正的文件夹父轨。

### 嵌套文件夹处理

当选中的文件夹存在父子嵌套时，脚本只使用“最外层的已选文件夹”判断下一次应该进入哪个状态，避免已经被父文件夹隐藏的子文件夹保留旧折叠值，从而导致循环跳级。

写入状态时则反过来，从最深层子文件夹开始，最后处理最外层父文件夹。这样父文件夹完全折叠之前，内部已选文件夹已经同步到相同状态；之后重新展开时不会出现父级展开但内部仍保持旧折叠状态的情况。

如果多个最外层已选文件夹原本就处于不同状态，脚本会以其中“折叠程度最高”的状态为基准推进一级，并把所有已选文件夹重新同步到同一级别。

### 推荐使用方式

建议绑定一个快捷键或工具栏按钮。框选多个文件夹轨后连续执行，即可统一轮换三个折叠级别。
