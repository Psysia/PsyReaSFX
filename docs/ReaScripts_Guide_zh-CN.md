...
脚本会根据当前选中的最外层文件夹内部是否存在下级文件夹，自动选择“两状态”或“四状态”模式。

#### 无嵌套文件夹：两状态

如果所有选中的最外层文件夹下面都只有普通轨道，没有任何下级文件夹父轨，则只在：
...
普通单层 Folder 会直接跳过“小折叠”等中间状态，更适合作为快速开关使用。

如果同时选中了多个单层文件夹，它们会始终统一状态。如果开始时状态不一致：

- 只要不是全部已经完全折叠，下一次就统一完全折叠；
- 当全部已经完全折叠时，下一次统一完全展开。

#### 存在嵌套文件夹：四状态

只要**任意一个**选中的最外层文件夹内部存在下级文件夹，整组选中的文件夹就自动使用：

```text
普通展开 → 小折叠 → 完全折叠 → 深度完全展开 → 普通展开
```

- **普通展开**：选中的文件夹父轨展开，未选中的内部文件夹保持自己的布局。
- **小折叠**：选中的文件夹父轨统一进入 REAPER 小折叠状态。
- **完全折叠**：选中的文件夹父轨统一完全折叠。
- **深度完全展开**：把所选最外层文件夹内部所有嵌套文件夹一起强制展开，让所有轨道层级都显示出来。

进入深度完全展开前，脚本会保存内部文件夹状态；回到普通展开时，会恢复未选中的内部文件夹布局，同时保持所有已选文件夹父轨展开。

### 推荐使用方式

仍然只需要绑定一个快捷键：单层文件夹时它就是“展开 / 折叠”开关；遇到嵌套文件夹时，会自动切换为四状态循环。
