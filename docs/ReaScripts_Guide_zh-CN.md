...
把所有选中的文件夹父轨作为一个整体，在四个状态之间循环：

```text
普通展开 → 小折叠 → 完全折叠 → 深度完全展开 → 普通展开
```

- **普通展开**：所有选中的文件夹父轨展开；未选中的内部嵌套文件夹保持自己原来的状态。
- **小折叠**：所有选中的文件夹父轨统一进入 REAPER 的小折叠状态。
- **完全折叠**：所有选中的文件夹父轨统一进入完全折叠状态。
- **深度完全展开**：不仅展开选中的文件夹，还会把这些文件夹内部包含的所有嵌套文件夹一起强制展开，让内部所有轨道层级都显示出来。

普通轨道即使被选中也不会修改。

### 嵌套文件夹处理

脚本会把多层嵌套的已选文件夹作为同一个整体处理。

进入**深度完全展开**前，会先记录内部各文件夹原来的折叠状态，然后再把整个所选文件夹子树全部展开。

下一次进入**普通展开**时：

- 所有被选中的文件夹父轨保持展开；
- 没有被选中的内部文件夹恢复到深度展开之前的状态。

因此“深度完全展开”可以临时查看整个文件夹内部结构，又不会永久破坏原本的嵌套折叠布局。

### 推荐使用方式

建议绑定一个快捷键或工具栏按钮。连续执行即可统一轮换四个折叠级别。
