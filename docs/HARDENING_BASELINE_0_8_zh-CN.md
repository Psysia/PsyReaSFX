...
`tools/generate_hardening_library.py` 可以从空目录确定性生成 1 到 500,000 条 WAV，包含：
...
```text
5,000：日常 PR 基准
50,000：真实文件扫描与 RC 交互门槛
500,000：合成目录容量与长时间压力门槛
```

生成器拒绝默认写入非空目录，所有夹具都只能放在一次性测试位置。50 万条完整
WAV 夹具会产生显著的文件系统开销，因此自动容量门槛使用不落地音频的确定性
合成目录；真实音频读取、波形和滚动仍由 50,000 条夹具单独验收。
...
4. Lua 25,000 条分帧结果重建、排序和筛选测试；
5. Lua 增量日志的代次、校验、截断和 10,000 条容量测试；
6. Desktop Release 构建（零警告、零错误）；
7. NuGet 已知漏洞审计；
8. Desktop 完整自检；
9. 一次性素材库生成与无界面索引/保存/加载基准；
10. Desktop 25,000 条合成目录容量冒烟检查。

RC 还必须在参考机上手工运行 500,000 条合成容量门槛。它验证首次 SQLite
建库、完整状态模型加载、线性搜索、完整性检查、数据库体积、托管内存和进程
峰值工作集；具体门槛与最近结果见 `docs/CAPACITY_STANDARD_500K_zh-CN.md`。
...
- 扫描中断、磁盘满、无权限和文件占用故障；
- 50,000 条真实文件库的滚动、试听、退出和恢复；
- 500,000 条目录在真实机器上的启动、搜索、筛选和退出；
