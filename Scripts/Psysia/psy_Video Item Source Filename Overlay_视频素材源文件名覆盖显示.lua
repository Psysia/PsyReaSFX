-- @description Video Item Source Filename Overlay / 视频素材源文件名覆盖显示
-- @version 1.0
-- @author Psysia
-- @requires js_ReaScriptAPI
-- @changelog
--   + Initial source-filename overlay release.
--   + Show each video's source filename without its file extension.
--   + Switch automatically with the track-name overlay.
--   + Share CJK rendering, multiline fitting, playback hiding, and font settings.

local PROJECT = 0
local EXT_SECTION = "PsysiaVideoItemFilenameOverlay"
local RUNNER_VERSION = "1.0"
local RUNNER_MODE = "source_filename"
...
if not api_ok then
    reaper.ShowMessageBox(
        ...
        "Video Item Source Filename Overlay / 视频素材源文件名覆盖显示",
        0
    )
    return
end
